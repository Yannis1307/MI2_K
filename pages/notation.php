...
<main>
    <div class="form-container">
        <div class="form-panel">
            ...
            <!-- titre et soustitre -->
            <h1 class="form-title">
                <?= $is_general_review ? "AVIS GÉNÉRAL SUR LE RESTAURANT" : "TRANSMISSION D'ÉVALUATION" ?></h1>
            ...
            <?php if (!empty($erreur_form)): ?>
                <div
                    style="background: rgba(255,50,50,0.15); border: 1px solid rgba(255,50,50,0.4); padding: 14px; border-radius: 10px; text-align: center; color: #ff8080; margin-bottom: 20px;">
                    ❌ <?= $erreur_form ?>
                </div>
            <?php endif; ?>
            ...
            <form class="rating-form" data-general-review="<?= $is_general_review ? 'true' : 'false' ?>"
                action="notation.php<?= !$is_general_review ? '?id=' . urlencode($id_commande) : '' ?>" method="POST">
                ...
                <?php if ($is_general_review): ?>
                    ...
                    <fieldset class="rating-fieldset">
                        ...
                        <div class="star-rating" id="stars-global">
                            <input type="radio" id="global-5" name="global-rating" value="5"
                                <?= (isset($_POST['global-rating']) && $_POST['global-rating'] == 5) ? 'checked' : '' ?>
                                required>
                            <label for="global-5" title="5 étoiles — Exceptionnel !">★</label>

                            <input type="radio" id="global-4" name="global-rating" value="4"
                                <?= (isset($_POST['global-rating']) && $_POST['global-rating'] == 4) ? 'checked' : '' ?>>
                            <label for="global-4" title="4 étoiles — Très bon">★</label>

                            <input type="radio" id="global-3" name="global-rating" value="3"
                                <?= (isset($_POST['global-rating']) && $_POST['global-rating'] == 3) ? 'checked' : '' ?>>
                            <label for="global-3" title="3 étoiles — Correct">★</label>

                            <input type="radio" id="global-2" name="global-rating" value="2"
                                <?= (isset($_POST['global-rating']) && $_POST['global-rating'] == 2) ? 'checked' : '' ?>>
                            <label for="global-2" title="2 étoiles — Moyen">★</label>

                            <input type="radio" id="global-1" name="global-rating" value="1"
                                <?= (isset($_POST['global-rating']) && $_POST['global-rating'] == 1) ? 'checked' : '' ?>>
                ...
                <fieldset class="rating-fieldset">
                    <legend class="rating-legend">
                        ...
                        <?= $is_general_review ? "Votre Avis" : "Commentaire de Transmission" ?>
                    </legend>
                    <p class="rating-hint">Ajoutez des détails à votre <?= $is_general_review ? "avis" : "rapport" ?>.
                    </p>
                ...
                <div id="js-notation-error"
                    style="display:none; background: rgba(255,50,50,0.15); border: 1px solid rgba(255,50,50,0.4); padding: 12px 16px; border-radius: 10px; color: #ff8080; text-align: center; margin-bottom: 10px;">
                </div>
